...
use crate::{error::Error, map::MapStats, run::GamePlayed, CodVersion, GunfightMap, DAY_FMT};
...
mod tests {
    ...
    fn test_stats_empty() -> Result<()> {
        // Arrange
        let games: Vec<GamePlayed> = Vec::new();

        // Act / Assert
        assert_eq!(
            Stats::new(&games, Local::now(),                &CodVersion::MW, )?,
            Stats {
